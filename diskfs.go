...
//		disk := diskfs.Create(diskImg, size, diskfs.SectorSizeDefault)
//
...
func initDisk(f fs.File, openMode OpenModeOption, sectorSize SectorSize) (*disk.Disk, error) {
	...
	newDisk.Backend = util.BackendFromFile(f, !writableMode(openMode))
...
// Use OpenOpt to control options, such as sector size or open mode.
func WithBackend(f fs.File, opts ...OpenOpt) (*disk.Disk, error) {
	opt := &openOpts{
		mode:       ReadOnly,
		sectorSize: SectorSizeDefault,
	}
...
// Obsolete: use <backend>.CreateFromPath + diskfs.OpenFile
...
func Create(device string, size int64, sectorSize SectorSize) (*disk.Disk, error) {
	rawBackend, err := raw.CreateFromPath(device, size)
	if err != nil {
		return nil, err
	}
	// return our disk
	return initDisk(rawBackend, ReadWriteExclusive, sectorSize)
}
